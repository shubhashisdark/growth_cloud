...
function assertFromAddress(value, providerName) {
    if (!value || value.trim() === "") {
...
function parseProviderName(value, fallback) {
...
export async function sendEmailWithFallback(message) {
    const { primary, fallback } = getEmailProviders();
    const errors = [];
    try {
        const receipt = await primary.send(message);
        return { ...receipt, attemptedProviders: [primary.name] };
    }
    catch (error) {
        errors.push({
    ...
    if (!fallback || fallback.name === primary.name) {
        ...
        throw new Error(`Email delivery failed via ${failure.provider}: ${failure.error}`);
    }
    try {
        const receipt = await fallback.send(message);
        return { ...receipt, attemptedProviders: [primary.name, fallback.name] };
    }
    catch (error) {
        errors.push({
    ...
    throw new Error(`Email delivery failed via ${errors.map((item) => item.provider).join(" and ")}: ${errors.map((item) => item.error).join(" | ")}`);
}
export function renderEmailTemplate(template, variables) {
    const replaceVariables = (value) => value.replace(/\{\{\s*([\w.-]+)\s*\}\}/g, (_match, key) => variables[key] ?? "");
    return {
...
export function buildTrackingUrls({ baseUrl, campaignId, messageId, recipientId }) {
...
export function buildWelcomeEmail(name, workspaceName) {
...
export function buildInvitationEmail(inviterName, workspaceName, inviteUrl) {
