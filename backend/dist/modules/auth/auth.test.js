...
test("Auth HTTP routes - POST /api/v1/auth/signup & login", async () => {
    ...
    // Signup HTTP
    const signupResponse = await app.fetch(new Request("http://127.0.0.1/api/v1/auth/signup", {
        method: "POST",
    ...
    // Login HTTP
    const loginResponse = await app.fetch(new Request("http://127.0.0.1/api/v1/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            email,
    ...
    const meResponse = await app.fetch(new Request("http://127.0.0.1/api/v1/auth/me", {
        method: "GET",
