...
import { createApiKey, rotateApiKey, revokeApiKey, authenticateApiKey } from "./api-key.service.js";
...
test("API Keys HTTP routes - create, authenticate, and usage telemetry", async () => {
    ...
    // Signup owner
    const signupRes = await app.fetch(new Request("http://127.0.0.1/api/v1/auth/signup", {
        method: "POST",
    ...
    // Login owner
    const loginRes = await app.fetch(new Request("http://127.0.0.1/api/v1/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    const createKeyRes = await app.fetch(new Request("http://127.0.0.1/api/v1/api-keys", {
        method: "POST",
        headers: {
    ...
    const authKeyRes = await app.fetch(new Request("http://127.0.0.1/api/v1/api-keys/authenticate", {
        method: "POST",
    ...
    const trackRes = await app.fetch(new Request(`http://127.0.0.1/api/v1/api-keys/${apiKey.id}/usage`, {
        method: "POST",
        headers: {
