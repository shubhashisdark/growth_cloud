...
async function executeAction(step, lead, runId) {
    ...
    switch (step.actionType) {
        ...
        case "add_tag": {
            ...
            if (!tag)
                return { success: false, output: {}, error: "No tag specified" };
        ...
        case "change_status": {
            ...
            await prisma.lead.update({ where: { id: lead.id }, data: { lifecycleStage: newStatus } });
        ...
        case "fire_webhook": {
            ...
            if (!url)
                return { success: false, output: {}, error: "No webhook URL specified" };
...
export async function executeWorkflow(workflowId, leadId, triggerEvent, input = {}) {
    ...
    try {
        for (const step of steps) {
            ...
            if (step.type === "condition") {
                if (!lead)
                    continue;
                const pass = evaluateCondition(lead, step);
                ...
                if (!pass)
                    break; // Condition failed — stop workflow
                continue;
            }
            // Delay step
            if (step.type === "delay") {
            ...
            if (step.type === "action" && lead) {
                ...
                while (attempt < MAX_RETRIES) {
                    try {
                        result = await executeAction(step, lead, run.id);
                        if (result.success)
                            break;
