...
import { createWorkspaceSchema, updateWorkspaceSchema, createMemberSchema, updateMemberRoleSchema, sendInvitationSchema } from "../validators.js";
...
export class WorkspacesController {
    workspacesService;
    constructor(workspacesService = new WorkspacesService()) {
        this.workspacesService = workspacesService;
    }
    list = async (request, response) => {
        const userId = request.actor?.userId;
        const result = await this.workspacesService.listWorkspaces(userId);
        return sendSuccess(response, result.data, { total: result.data.workspaces.length });
    };
    ...
    listMembers = async (request, response) => {
        ...
        return sendSuccess(response, result.data, { total: result.data.items.length });
    };
    createMember = async (request, response) => {
        const workspaceId = getParam(request.params.workspaceId);
    ...
    sendInvitation = async (request, response) => {
        const workspaceId = getParam(request.params.workspaceId);
        const actorUserId = request.actor?.userId || "";
    ...
    listInvitations = async (request, response) => {
        const workspaceId = getParam(request.params.workspaceId);
        const result = await this.workspacesService.listInvitations(workspaceId);
        return sendSuccess(response, result.data, { total: result.data.items.length });
    };
    revokeInvitation = async (request, response) => {
        const workspaceId = getParam(request.params.workspaceId);
