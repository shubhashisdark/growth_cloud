...
test("Workspace HTTP routes - authentication & membership guard", async () => {
    ...
    const unauthRes = await app.fetch(new Request(`http://127.0.0.1/api/v1/workspaces/${workspaceId}`, { method: "GET" }));
    assert.equal(unauthRes.status, 401);
    // Authenticated member request should succeed
    const authRes = await app.fetch(new Request(`http://127.0.0.1/api/v1/workspaces/${workspaceId}`, {
        method: "GET",
