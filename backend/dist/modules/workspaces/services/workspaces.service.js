...
export class WorkspacesService {
    ...
    async createWorkspace(ownerId, input) {
        ...
        if (!workspaceSlug)
            workspaceSlug = `workspace-${randomUUID().slice(0, 6)}`;
    ...
    async updateWorkspace(workspaceId, input, actorUserId) {
    ...
    async createMember(workspaceId, input) {
    ...
    async removeMember(workspaceId, memberUserId, actorUserId) {
        const workspace = await prisma.workspace.findUnique({ where: { id: workspaceId } });
        if (!workspace) {
            return { error: { code: "NOT_FOUND", message: "Workspace not found" }, status: 404 };
        }
    ...
    async updateMemberRole(workspaceId, memberUserId, input, actorUserId) {
        const member = await prisma.workspaceMember.findUnique({
            where: { workspaceId_userId: { workspaceId, userId: memberUserId } }
        });
        if (!member) {
    ...
    async sendInvitation(workspaceId, actorUserId, input) {
        const workspace = await prisma.workspace.findUnique({ where: { id: workspaceId } });
        if (!workspace) {
            return { error: { code: "NOT_FOUND", message: "Workspace not found" }, status: 404 };
        }
    ...
    async revokeInvitation(workspaceId, invitationId, actorUserId) {
