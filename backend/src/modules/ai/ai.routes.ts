...
const promptTypeValues = ["email-generator", "lead-summary", "lead-scoring", "campaign-insights", "segment-builder"] as const;

type PromptType = (typeof promptTypeValues)[number];

type AiRunStatus = "completed" | "streaming" | "failed";
...
function buildPrompt(type: PromptType, input: Record<string, unknown>) {
  switch (type) {
...
async function createPromptLog(workspaceId: string, type: PromptType, prompt: string, status: AiRunStatus, output: string, attempts: number) {
...
async function runAiPrompt(workspaceId: string, type: PromptType, input: Record<string, unknown>, stream: boolean, retries: number) {
...
function parseAiInput(type: PromptType, input: Record<string, unknown>) {
  switch (type) {
    case "email-generator":
...
aiRouter.post("/run", requireAuth, requireWorkspaceMember(), async (request, response) => {
  ...
  const result = await runAiPrompt(request.workspace!.workspaceId, payload.type, parsedInput as Record<string, unknown>, payload.stream, payload.retries);
