...
const createApiKeySchema = z.object({
  ...
  name: z.string().min(2),
  type: z.enum(["public", "secret"] satisfies [ApiKeyType, ApiKeyType]),
...
function jsonSuccess(response: Response, data: unknown, status = 200) {
  return response.status(status).json({ data, meta: { timestamp: new Date().toISOString() }, error: null });
}

function jsonError(response: Response, status: number, code: string, message: string) {
...
async function resolveContext(
  ...
  bodyActorId?: string
): Promise<{ actorId: string; workspaceId: string; role: string } | { error: string; message: string; status: number }> {
  const actorId = bodyActorId || request.actor?.userId;
  const workspaceId = bodyWorkspaceId || (request.query?.workspaceId as string) || (request.headers?.["x-workspace-id"] as string);

  if (!actorId) return { error: "UNAUTHORIZED", message: "Authentication is required", status: 401 };
  if (!workspaceId) return { error: "WORKSPACE_ID_REQUIRED", message: "workspaceId is required", status: 400 };

  const membership = await prisma.workspaceMember.findUnique({ where: { workspaceId_userId: { workspaceId, userId: actorId } } });
  if (!membership) return { error: "FORBIDDEN", message: "Not a member of target workspace", status: 403 };
...
apiKeysRouter.post("/authenticate", async (request, response) => {
  ...
  if (!authenticated) return jsonError(response, 401, "UNAUTHORIZED", "Invalid or expired API key");
...
apiKeysRouter.get("/", requireAuth, async (request, response) => {
  ...
  if ("error" in ctx) return jsonError(response, ctx.status, ctx.error, ctx.message);
...
apiKeysRouter.post("/", requireAuth, async (request, response) => {
  ...
  if (!canManageApiKeys(ctx.role)) return jsonError(response, 403, "FORBIDDEN", "Insufficient permissions to manage API keys");
...
apiKeysRouter.post("/:apiKeyId/usage", async (request, response) => {
  const authenticated = await authenticateApiKey(request.headers.authorization);
  if (!authenticated) return jsonError(response, 401, "UNAUTHORIZED", "Invalid API key");
