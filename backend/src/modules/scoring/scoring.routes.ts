...
scoringRouter.patch("/rules/:ruleId", requireAuth, requireWorkspaceMember(), async (req, res) => {
  ...
  if (!existing) return res.status(404).json({ data: null, error: { code: "NOT_FOUND", message: "Rule not found" }, meta: {} });
...
scoringRouter.post("/recalculate/:leadId", requireAuth, requireWorkspaceMember(), async (req, res) => {
  const result = await recalculateLeadScore(req.params.leadId as string);
...
scoringRouter.get("/history/:leadId", requireAuth, requireWorkspaceMember(), async (req, res) => {
  ...
  res.json({
    data: {
      items: history.map((h: { id: string; leadId: string; ruleId: string | null; scoreBefore: number; scoreAfter: number; delta: number; reason: string; createdAt: Date; rule?: { name: string; type: string } | null }) => ({
        id: h.id,
        leadId: h.leadId,
        ruleId: h.ruleId,
