...
function getStartDateFromRange(range: DateRange, customStartDate?: string): Date | null {
  if (customStartDate) {
...
export async function getAnalyticsOverview(
  ...
  const timeseries: Array<{ date: string; leads: number; emailsSent: number; workflowRuns: number }> = [];
  ...
  const aiInsights: Array<{ title: string; type: "positive" | "warning" | "info"; description: string; recommendation: string }> = [];
...
export async function exportAnalyticsData(workspaceId: string, format: "json" | "csv", range: DateRange = "30d") {
